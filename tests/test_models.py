...
def test_payment_table_creator_get_all(objs):
    ...
    ptables = objs.ptable_creator.get_all(conn=objs.conn)
    check = True
...
def test_earning_save_update(objs):
    ...
    new_earning = objs.earning_creator.get(objs.earning.id, conn=objs.conn)
    assert objs.earning.value == new_earning.value


def test_earning_creator_get(objs):
    """Test if EarningCreator get method works"""
    earning = objs.earning_creator.get(1, conn=objs.conn)
...
def test_earning_creator_get_all(objs):
    ...
    earnings = objs.earning_creator.get_all(conn=objs.conn)
    check = True
...
def test_perhourpayment_save_update(objs):
    ...
    new_payment = objs.perhourpayment_creator.get(
        objs.perhourpayment.id, conn=objs.conn
    )
...
def test_perhourpayment_creator_get(objs):
    ...
    perhourpayment = objs.perhourpayment_creator.get(1, conn=objs.conn)
...
def test_perhourpayment_creator_get_all(objs):
    ...
    payments = objs.perhourpayment_creator.get_all(conn=objs.conn)
    check = True
